...
// FBA GFX Buffer probably different depending on the system
static unsigned char fbagfx[1024 * 512 * 2];

// Memory ROM Loading
extern unsigned int FBATimer;
...
{
    /* X, Y, Z   Values set are for roughly 4:3 aspect ratio */
    -HASPECT, VASPECT, 0,       // 0
    HASPECT, VASPECT, 0,        // 1
    HASPECT, -VASPECT, 0,       // 2
    -HASPECT, -VASPECT, 0,      // 3
...
#endif // endif VIDEO_THREADING

static void copy_to_xfb(u32 arg){
    if ( copynow == GX_TRUE)
    {
...
unsigned int prevRenderedFrameCount = 0;
bool CheckVideo;
...
void setGFX ()
{
        //GFX.Screen = (unsigned short *) fbagfx;
        //GFX.Pitch = 1024;
}
