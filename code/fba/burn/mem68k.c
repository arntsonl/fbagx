...
char m68k_readmap_8[32];
char m68k_readmap_16[32];
char m68k_writemap_8[32];
char m68k_writemap_16[32];

// We want to hack this in from the real final burn, NOT genplus
...
unsigned int m68k_read_memory_8(unsigned int address)
{
	...
	switch (m68k_readmap_8[offset])
	{
			...
			if (base <= 0xa1ff)
			{
				switch (base & 0xff)
				{
					case 0x00:	/* I/O chip */
						...
						return (-1);//io_read((address >> 1) & 0x0f));
		...
		case VDP:
			...
			if (((address >> 16) & 0x07) == 0) 
			{
				switch (address & 0xff)
			  {
				  case 0x00:		/* DATA */
				  case 0x02:
				   	return (-1);//vdp_data_r() >> 8);
			
				  case 0x01:		/* DATA */
				  case 0x03:
				  	return (-1);//vdp_data_r() & 0xff);
			
				  case 0x04:		/* CTRL */
				  case 0x06:
				    return ((m68k_read_bus_8(address) & 0xfc) | ((0)));//vdp_ctrl_r() >> 8) & 3));

				  case 0x05:		/* CTRL */
				  case 0x07:
				   	return (-1);//vdp_ctrl_r() & 0xff);
			
				  case 0x08:		/* HVC */
				  case 0x0a:
				  case 0x0c:
				  case 0x0e:
				   	return (-1);//vdp_hvc_r() >> 8);
			
				  case 0x09:		/* HVC */
				  case 0x0b:
				  case 0x0d:
				  case 0x0f:
				   	return (-1);//vdp_hvc_r() & 0xff);
      ...
      switch (address & 0xff)
      {
        ...
        {
          uint8 retval = 0xff;
		  // Hacked out for our Final Burn Alpha
		  /*
        ...
        case 0x05:  /* MSB PEN X coordinate */
          //return (input.analog[0][0] >> 8);
			// Hacked out for final burn alpha
			return -1;
		  ...
		  if (base <= 0xa0ff) 
	    {
			  ...
			  if (zbusack) return m68k_read_bus_16(address);
				...
				switch (base & 0x60)
				{
					case 0x40: /* YM2612 */
					{
						int temp = 0;//fm_read(0, address & 3);
					  return (temp << 8 | temp);
					}
			...
			if (base <= 0xa1ff)
			{
				switch (base & 0xff)
				{
	    			...
	    			int temp = 0;//io_read((address >> 1) & 0x0f);
	      		return (temp << 8 | temp);
					}
		...
		case VDP:
			...
			if (((address >> 16) & 0x07) == 0) 
			{
				switch (address & 0xfc)
			  {
				  ...
				  case 0x04:	/* CTRL */
				   	return 0;//vdp_ctrl_r() & 0x3FF) | (m68k_read_bus_16(address) & 0xFC00));
