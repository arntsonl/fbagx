...
void SN76496Update(int Num, short* pSoundBuf, int Length)
{
	...
	while (Length > 0)
	{
		...
		if (Num > 0) {
			Temp[0] = pSoundBuf[0] + Out;
			if (Num == NumChips) {
				if (Temp[0] > 32767) Temp[0] = 32767;
				if (Temp[0] < -32768) Temp[0] = -32768;
			}
			pSoundBuf[0] = Temp[0];
			
			Temp[1] = pSoundBuf[1] + Out;
			if (Num == NumChips) {
				if (Temp[1] > 32767) Temp[1] = 32767;
				if (Temp[1] < -32768) Temp[1] = -32768;
			}			
			pSoundBuf[1] = Temp[1];
		} else {
			pSoundBuf[0] = Out;
...
static void GenericStart(int Num, int Clock, int FeedbackMask, int NoiseTaps, int NoiseInvert)
{
	if (Num >= MAX_SN76496_CHIPS) return;
...
void SN76489Init(int Num, int Clock)
{
	return GenericStart(Num, Clock, 0x4000, 0x03, 1);
}

void SN76489AInit(int Num, int Clock)
{
	return GenericStart(Num, Clock, 0x8000, 0x06, 0);
}

void SN76494Init(int Num, int Clock)
{
	return GenericStart(Num, Clock, 0x8000, 0x06, 0);
}

void SN76496Init(int Num, int Clock)
{
	return GenericStart(Num, Clock, 0x8000, 0x06, 0);
}
